...
public:
    ...
    bool TellDebug(Player* player, std::string text, std::string strategy = "debug", BotState state = BotState::BOT_STATE_NON_COMBAT)
    {
        if (text.rfind("[PBTRACE]", 0) == 0 || HasStrategy(strategy, state))
            return TellPlayerNoFacing(player, text, PlayerbotSecurityLevel::PLAYERBOT_SECURITY_ALLOW_ALL, true, true);
        return false;
    }
